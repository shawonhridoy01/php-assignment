...
if(isset($_POST["submit"])){
    ...
    // $emailChecker = filter_var($email,FILTER_VALIDATE_EMAIL);
    $password = mysqli_real_escape_string($conn,$_POST["password"]);
    
    $insertQuery = "select * from registration where email = '{$email}'";
    $insertQueryResult = mysqli_query($conn,$insertQuery) or die("query error");
    $loggedAbility = false;
    // $emailChecker = true;
    
    $numOfRow = mysqli_num_rows($insertQueryResult);
    if($numOfRow > 0){
        $emailChecker = true ;
        while($row= mysqli_fetch_assoc($insertQueryResult)){
    ...
    }elseif($emailChecker == false){
        $message =" Invalid Email Try again";
    }
    
    elseif($loggedAbility == false){
